...
| `HTTPS_ENABLED` | `false` | 是否让 `python -m app.run` 使用 HTTPS 端口。 |
| `HTTPS_PORT` | `8443` | HTTPS 模式端口。 |
| `HTTPS_CERTIFICATE_SOURCE` | `self-signed` | HTTPS 证书来源：`self-signed`、`path` 或 `acme`。 |
...
生产部署不建议在 SSH 会话里直接长期运行 `uvicorn`。推荐做法是优先使用 Docker Compose 管理 API 和 PostgreSQL；如果不使用 Docker，也可以用 systemd 管理后端进程，公网直接访问应用监听的 HTTP 或 HTTPS 端口。
...
- `ohmstash-api`：FastAPI 应用，内置 Web UI。
- `ohmstash-db`：PostgreSQL 16，数据保存在 Docker volume 中。
- `ohmstash-caddy`：Caddy 反向代理，可自动申请 ACME 证书。
...
当前 Compose 映射了 `8000:8000` 和 `8443:8443`。建议容器内部端口保持 `HTTP_PORT=8000`、`HTTPS_PORT=8443`；如果只想修改公网访问端口，改 `docker-compose.yml` 中冒号左侧的主机端口即可，例如 `18000:8000`。

Compose 额外映射了 Caddy 的 `80:80`、`443:443` 和 `443:443/udp`。默认 `HTTPS_ENABLED=false` 时，Caddy 会在 80 端口反向代理到 API。启用 ACME 时，进入「设置 -> 服务」，选择证书来源 `ACME`：

- `HTTP-01` 会强制保存为 `HTTP_PORT=80`、`HTTPS_PORT=443`。
...
没有证书路径时应用会在 `/app/certs` 自动生成自签证书，Compose 已将该目录持久化到 `ohmstash-certs` volume。

Docker 部署中的持久化数据：

- PostgreSQL 数据：`ohmstash-postgres-data`。
- 应用日志：`ohmstash-logs`，容器内路径 `/app/logs/ohmstash.log`。
- HTTPS 证书：`ohmstash-certs`，容器内路径 `/app/certs`。
- Caddy 证书和运行状态：`ohmstash-caddy-data`、`ohmstash-caddy-config`。
- Caddyfile：宿主机项目目录下的 `caddy/Caddyfile` 会挂载给 Caddy，并由 API 在保存 ACME 设置时更新。
- 服务配置：宿主机项目目录下的 `.env.docker` 会挂载为容器内 `/app/.env`，因此在 UI 中保存的服务端口和证书路径会写回这个文件。
