...
from app.schemas.vlm_provider_config import (
    VlmProviderConfigCreate,
    VlmProviderConfigUpdate,
)


class CRUDVlmProviderConfig(
    CRUDBase[VlmProviderConfig, VlmProviderConfigCreate, VlmProviderConfigUpdate]
):
    def get_by_name(self, db: Session, *, name: str) -> Optional[VlmProviderConfig]:
        return db.query(self.model).filter(self.model.name == name).first()

    def get_default(self, db: Session) -> Optional[VlmProviderConfig]:
    ...
    def create(
        ...
        db: Session,
        *,
        obj_in: VlmProviderConfigCreate,
    ) -> VlmProviderConfig:
    ...
    def update(
        ...
        db: Session,
        *,
        db_obj: VlmProviderConfig,
        obj_in: VlmProviderConfigUpdate,
    ) -> VlmProviderConfig:
...
vlm_provider_config = CRUDVlmProviderConfig(VlmProviderConfig)
