...
function renderRecognitionSessionList() {
    ...
    list.innerHTML = filteredSessions.map((session) => {
        ...
        const statusClass = escapeClassToken(session.status || "unknown");
        const verificationClass = escapeClassToken(session.verification_status || "unknown");
        return `
            <article class="recognition-session-card${active}">
                <div class="recognition-session-main">
                    ...
                    <div class="recognition-session-status">
                        <span class="session-badge status-${statusClass}">${escapeHtml(statusLabel)}</span>
                        ${verificationLabel ? `<span class="session-badge verification-${verificationClass}">${escapeHtml(verificationLabel)}</span>` : ""}
                    </div>
                </div>
