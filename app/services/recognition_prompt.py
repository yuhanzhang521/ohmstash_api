...
COMPONENT_NAME_RULE_TEXT = (
    "name 必须让人一眼看出器件是什么，不要只使用标签第一行的参数。"
    "基础被动元件（电阻、电容、电感）优先使用“数值 + 封装”作为 name，"
    "其中封装只用于贴片元件，插件元件不要强行写封装，例如 10k 0603。"
    "集成电路、芯片、模块上的主芯片优先提取完整型号作为 name，"
    "例如 STM32F103C8T6、MAX98357A。"
    "功能模块、机电件、组件、风扇、传感器、开关等 name 必须包含功能性名词；"
    "如果标签分两行且一行是参数、另一行是器件名词，请优先使用器件名词，"
    "必要时保留关键参数但保持简洁。"
    "例如“12V 5015 / 离心风扇”应写为“12V 离心风扇”，"
